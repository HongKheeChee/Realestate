...
if(isset($_POST['add']))
{
    ...
    // Move uploaded files only if they have valid extensions
    ...
    $aimageExtension = strtolower(pathinfo($aimage, PATHINFO_EXTENSION));
    $aimage1Extension = strtolower(pathinfo($aimage1, PATHINFO_EXTENSION));
    $aimage2Extension = strtolower(pathinfo($aimage2, PATHINFO_EXTENSION));
    $aimage3Extension = strtolower(pathinfo($aimage3, PATHINFO_EXTENSION));
    $aimage4Extension = strtolower(pathinfo($aimage4, PATHINFO_EXTENSION));
    $fimageExtension = strtolower(pathinfo($fimage, PATHINFO_EXTENSION));
    $fimage1Extension = strtolower(pathinfo($fimage1, PATHINFO_EXTENSION));
    $fimage2Extension = strtolower(pathinfo($fimage2, PATHINFO_EXTENSION));

    // Check if the file extensions are in the allowed list
    if (!in_array($aimageExtension, $allowedExtensions) ||
        !in_array($aimage1Extension, $allowedExtensions) ||
        !in_array($aimage2Extension, $allowedExtensions) ||
        !in_array($aimage3Extension, $allowedExtensions) ||
        !in_array($aimage4Extension, $allowedExtensions) ||
        !in_array($fimageExtension, $allowedExtensions) ||
        !in_array($fimage1Extension, $allowedExtensions) ||
        !in_array($fimage2Extension, $allowedExtensions)) {
        $error = "<p class='alert alert-warning'>Invalid file format. Allowed formats: JPG, JPEG, PNG.</p>";
    } else {
        // Move uploaded files only if they have valid extensions
        move_uploaded_file($temp_name, "admin/property/$aimage");
        move_uploaded_file($temp_name1, "admin/property/$aimage1");
